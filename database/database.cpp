...
Database::Database(const std::string& database_name, const std::string& i_table_name)
...
int64_t Database::__read_last_id__(const std::string& i_table_name) {
  ...
  sqlite3_step(this->m_db_statement);
  int64_t last_id = sqlite3_column_int64(this->m_db_statement, 0);
  TRC("Read last id [%li] from table ["%s"].", last_id, i_table_name.c_str());
...
void Database::__where_check__(const int64_t& i_id) {
