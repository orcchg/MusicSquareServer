...
#ifndef MSQ_DATABASE__H__
#define MSQ_DATABASE__H__
...
#define DATABASE_NAME "MusicSquareServerDatabase.db"
...
class Database {
protected:
  Database(const std::string& database_name = DATABASE_NAME, const std::string& table_name = "Default_Table");
...
protected:
  ...
  int64_t __read_last_id__(const std::string& table_name);
  void __drop_table__(const std::string& table_name);
  void __vacuum__();

#if ENABLED_ADVANCED_DEBUG
  void __where_check__(const int64_t& id);
  int __count_check__();
#endif
